...
class Datasett extends CI_Controller{
public function __construct()
    {
        parent::__construct();
        $this->load->model('Mod_dataset');
        $this->load->dbforge();
    }
    ...
    {
        $data['kategori'] = $this->Mod_dataset->getAll_kat()->result();
        $data['jenis'] = $this->Mod_dataset->getAll_list()->result();
        if($this->uri->segment(3)=="create-success") {
			...
			$this->load->view('dataset/data_set2',$data);
        }
...
/*
    public function input(){
    	$this->template->load('layoutbackend','dataset/data_input_set');
    }
*/
    public function insert()
    {
        ...
        if (isset($_POST['save'])) {
           ...
           if ($this->form_validation->run()==true) {
               ...
               $cek=$this->Mod_dataset->cekjenis($jenis);
               if ($cek->num_rows() > 0) {
                ...
                $save_list = array('jenis' =>$jenis ,
                              'kategori' => $kategori,
                              'keaktifan'=> 1 );
                $this->Mod_dataset->insert_list("datasetku",$save_list);
                    ...
                    if ($key['total'] > 0) {
                        ...
                        $save_s=$this->db->query($save_count);
                         ...
                         $save_stem = array('stem' => $value,
                                        ...
                                        'keaktifan'=>1 );
                        $this->Mod_dataset->insert_stem('stem',$save_stem);
                    }
                }
/*
                }
                for ($i=0; $i < count($separate) ; $i++) { 
                    ...
                    $cek=$this->Mod_dataset->cek_stem('stem',$ceknya);
...
*/
                ...
                redirect('datasett/index/create-success');
               }
           }
           else{
                $data['message'] = "";
    ...
    public function get_kategori()
    {
        ...
        if(isset($_POST['add'])){
			...
			$this->load->view('dataset/data_inputt',$data);
            }      
