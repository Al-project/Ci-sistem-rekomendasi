...
class Model_alternatif extends CI_Model {
    public function tambah_alternatif($data) {
        ...
        $filter = $this->db->select('*')->from('tbl_alternatif')->where('kode_uji', $data['kode_uji'])->get()->num_rows();
    ...
    public function edit_alternatif($data) {
        //Lakukan Update Data
        $check = $this->db->set($data)->where('kode_uji', $data['kode_uji'])->update('tbl_alternatif');
    ...
    public function hapus_alternatif($data) {
        ...
        $check = $this->db->delete('tbl_alternatif', array('kode_uji' => $data['kode_uji']));
        //Check jika telah berhasil menghapus alternatif
        if ($check) {
            //Pesan Response
            $response = array(
                'status' => 'success',
                'message' => 'Data Alternatif Berhasil Di Hapus'
            );
            return json_encode($response);
    ...
    public function data_edit($data){
        ...
        $this->db->where('tbl_evaluasi.kode_uji',$data['kode_uji']);
        return $this->db->get();
    }
    ...
    public function data_calc_missCalc(){
        ...
        foreach($tbl_alternatif->result() as $row_alternatif){
            $data_json[$x]['kode_uji']=$row_alternatif->kode_uji;
            $data_json[$x]['nama_lengkap']=$row_alternatif->nama_lengkap;
            foreach($tbl_kriteria->result() as $row_kriteria){
                ...
                $this->db->where('tbl_evaluasi.kode_uji',$row_alternatif->kode_uji);
    ...
    public function data_calc(){
        ...
        for($x=0;$x<count($tbl_alternatif);$x++){
            $data_wp[$x]['kode_uji'] = $tbl_alternatif[$x]['kode_uji'];
            $data_wp[$x]['nama_lengkap'] = $tbl_alternatif[$x]['nama_lengkap'];
        }
        ...
        for($x=0;$x<count($tbl_alternatif);$x++){
            ...
            $this->db->select('*');
            $this->db->from('tbl_evaluasi');
            $this->db->where('kode_uji',$tbl_alternatif[$x]['kode_uji']);
